...
It holds what the code cannot say: current state of ongoing efforts, what was tried and abandoned and why, syntheses worth finding again, and project facts that conversation keeps re-establishing. Code knowledge stays in the repo when the project documents it (`docs/`, README, CLAUDE.md) — link to those pages rather than copying them. In a project without such docs, the Logbook is the knowledge repository, code orientation included — write those pages knowing the code moves, and read them trusting the timestamps.

## The shape

- **Pages** are slug-addressed markdown, whole-field writes. Kinds: `saga` (a long-running effort: current state, attempts, open issues), `synthesis` (an audit, review, or investigation result), `reference` (stable facts). Statuses: `active` · `settled` · `archived` — only active pages reach the launch digest. Each page carries a one-line `summary` (the index/digest row) plus a short `description` — a couple of sentences on what the page is for and when to read it — so a listing explains itself without opening every page.
