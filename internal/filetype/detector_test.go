...
func TestDetector_DetectModeline(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "vim shell filetype",
			content:  "# vim: ft=shell\necho hello",
			wantLang: "shell",
			wantOk:   true,
		},
		{
			name:     "vim bash filetype",
			content:  "# vim: ft=bash\necho hello",
			wantLang: "shell",
			wantOk:   true,
		},
		{
			name:     "vim powershell filetype",
			content:  "# vim: ft=powershell\nWrite-Host hello",
			wantLang: "powershell",
			wantOk:   true,
		},
		{
			name:     "emacs powershell mode",
			content:  "# -*- mode: powershell -*-\nWrite-Host hello",
			wantLang: "powershell",
			wantOk:   true,
		},
		{
...
func TestDetector_DetectContent(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "shell if with test",
			content:  "if [ -f /etc/passwd ]; then\n  cat /etc/passwd\nfi",
			wantLang: "shell",
			wantOk:   true,
		},
		{
			name:     "shell case statement",
			content:  "case $1 in\n  start) echo Starting ;;\n  stop) echo Stopping ;;\nesac",
			wantLang: "shell",
			wantOk:   true,
		},
		...
		{
			name:     "powershell Get cmdlet",
			content:  "Get-ChildItem -Path C:\\ | Where-Object Name -Match 'test'",
			wantLang: "powershell",
			wantOk:   true,
		},
		{
			name:     "powershell param declaration",
			content:  "param(\n  [string]$Name,\n  [int]$Count = 10\n)\nWrite-Output $Name",
			wantLang: "powershell",
			wantOk:   true,
		},
