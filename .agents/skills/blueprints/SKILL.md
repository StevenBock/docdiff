...
Edit at the scale of the change. Use `hangar_patch_blueprint_body` for small doc/page/stylesheet edits; reserve whole-field setters for restructures. During wave planning, update only the active acceptance `##` section with `hangar_set_blueprint_acceptance(scope: ...)`. `hangar_check_blueprint_step` remains the one-item tick path; address acceptance items by exact `section` + phase-local `ordinal`. Trust the content hashes your own writes return — never re-download to verify your own mutation. To pick up a change you did NOT make, call `hangar_get_blueprint` (default summaries) and compare its content hashes against what you already hold; re-read ONLY the artifacts whose hash drifted — one via `hangar_get_blueprint(body: "<slug>")`, or a fresh download if many drifted.

## Drafting — planning with the human

1. `hangar_download_blueprint` ONCE at kickoff (or when a phase transition resets your context), then read the returned JSON file and ground everything in the ACTUAL repo — cite real files and verify claims against the code before they go in the doc. For later refreshes, hash-check with `hangar_get_blueprint` (default summaries) and re-read only artifacts whose content hash drifted; `hangar_download_blueprint(known_hash: <your last sha256>)` returning `unchanged: true` means your file is current, so do not re-read it.
