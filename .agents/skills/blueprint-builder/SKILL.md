...
Every write is whole-field and returns a content hash — trust it, and never re-download to verify your own mutation. To pick up a change you did NOT make, call `hangar_get_blueprint` (default summaries), compare its content hashes against what you hold, and re-read ONLY the artifacts that drifted — one via `hangar_get_blueprint(body: "<slug>")`, or a fresh `hangar_download_blueprint` if many drifted. `hangar_download_blueprint(known_hash: <your last sha256>)` returning `unchanged: true` means your file is current — do not re-read it. The exception is `hangar_check_blueprint_step`, which ticks one item in place; address acceptance items by exact phase `section` + local `ordinal`, and always prefer it over resending a whole plan or acceptance body.

## Building — executing a blueprint

1. Call `hangar_download_blueprint` ONCE at kickoff (re-download at a phase transition, or when a summaries hash-check shows many artifacts drifted), then read the returned JSON file in full: doc, brief, notes, acceptance criteria, and every linked page. At kickoff, ensure it has primary Logbook context: when `logbookPageSlug` is empty, create or choose an effort page (`hangar_logbook_upsert_page` when needed) and link it with `hangar_set_blueprint_logbook_page` before implementation.
